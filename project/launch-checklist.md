...
- [ ] Tag first releases to trigger image publishing (`giftcard-v1.0.0`, `key-monitor-v1.0.0`)
...
- [ ] Invite app decryption — hiveinvite.com/invite/ correctly decrypts multi-tenant payloads
- [ ] Restore app — hiveinvite.com/restore/ recovers keys from QR backup
- [ ] Telegram operator notifications — alert fires when new issuer application is submitted
- [ ] Key monitor alerts — unexpected operations on watched account trigger Telegram alerts

## Security

- [ ] Key monitor operational before accepting any delegating issuers (hard blocker)
- [ ] JWT secret set to a strong random value in production
- [ ] Service account active key stored only in Fly.io secrets, not in repo
- [ ] Allowed providers whitelist configured (no open registration without admin approval)
- [ ] Rate limiting on /claim and /validate verified working

## Content / Announcement

- [ ] Review landing page copy at hiveinvite.com
- [x] Draft Hive announcement post (what HiveInvite is, how issuers apply, what invitees get)
- [ ] Decide initial issuers / launch partners and pre-approve if needed
- [ ] Ensure service account has sufficient `create_claimed_account` tokens
- [ ] Ensure sufficient HP delegated for Resource Credits on created accounts
