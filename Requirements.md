...
1. **Endpoint feed enrollment timing.** After account creation, the gift card service enrolls the new user in the endpoint feed via a transfer to `haa-service`. But there is a gap between enrollment and the next feed update. Questions: (a) How long can the user rely on the proxy endpoints embedded in the gift card before the feed provides fresh endpoints? (b) What happens if those embedded endpoints rotate or go offline before enrollment completes? (c) Should the invite app or wallet verify that enrollment succeeded, or is fire-and-forget acceptable? The embedded endpoints may need a guaranteed validity window (e.g. 30 days) to bridge this gap.

2. **Invite app hosting accessibility.** The robust invite QR points to a URL hosting the invite app (e.g. GitHub Pages). But robust invites target regions where internet access is restricted. Questions: (a) Is GitHub Pages actually accessible in all target regions (China, Iran, etc.)? If not, the invite app URL is a single point of failure. (b) Should the QR support multiple fallback URLs (e.g. CDN mirrors)? (c) Could the invite app HTML be small enough to embed as a data URI in the QR itself? The current invite app (~50–80KB) exceeds practical QR capacity (~3KB for alphanumeric), so this is unlikely without extreme compression. (d) Could the invite app be distributed via alternative channels (Telegram file, USB, local network) for regions where no public URL is accessible?

3. **Proxy endpoint exposure and isolation.** The encrypted gift card payload contains proxy endpoint URLs. Questions: (a) Are these per-user endpoints, per-batch endpoints, or shared across all robust invites? (b) If all cards in a batch share the same proxy endpoints, one compromised card exposes infrastructure for the entire batch. (c) Should the gift card service issue per-user endpoint URLs at claim time (swapping the batch endpoints for personalized ones), so the embedded endpoints are only used during onboarding and not for ongoing wallet access? (d) What is the blast radius if an endpoint is leaked — can it be rotated without disrupting other users?

4. **Robust invite success screen UX.** The current success screen is entirely standard-invite focused (HiveSigner, peakd.com copy-paste flow). The robust flow needs different UX: no HiveSigner, no peakd.com — instead, a bootstrap file save prompt, a wallet-loading progress indicator, and messaging appropriate for a restricted-internet context. Questions: (a) Should this be a separate screen (`success-robust`) or conditional content within the existing success screen based on `payload.variant`? (b) What does the "congratulations" experience look like — wallet loading may take 10–30 seconds while chunks are fetched, so the success screen doubles as a loading screen. (c) Should the bootstrap file save be blocking (user must save before proceeding) or optional with a reminder?
...
6. **Offline/degraded network during robust invite flow.** Robust invites target users with restricted internet, so partial connectivity is a likely scenario. Questions: (a) What if the proxy endpoints from the gift card are also blocked at claim time? Should the invite app support manual endpoint entry as a fallback? (b) Should there be retry logic with backoff for transient proxy failures? (c) What is the UX when the gift card service is unreachable through the proxy — does the user see a generic error or specific guidance? (d) Should the invite app cache intermediate state (e.g. after verification but before claiming) so the user can resume if connectivity drops mid-flow?

7. **Gift card generation pipeline for robust batches.** The existing `giftcard-generate.ts` script needs to support robust invite generation. Questions: (a) How are proxy endpoint URLs provided as input — CLI flags, config file, or fetched from infrastructure? (b) Does the generation pipeline need access to the proxy infrastructure to validate endpoints are live before embedding them in cards? (c) How is the `locale` field determined — per-batch CLI flag matching the wallet locale codes? (d) Should the batch declaration `custom_json` include additional fields for robust batches (e.g. locale, endpoint count)?
