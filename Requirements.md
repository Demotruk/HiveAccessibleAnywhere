...
7. **Gift card generation pipeline for robust batches — PARTIALLY RESOLVED.** The existing `giftcard-generate.ts` script needs to support robust invite generation. The current script reads endpoints from a local `feed-config.json` file, but has no concept of variant, locale, or dedicated endpoint pools. Sub-questions (a), (b), and (c) are resolved; (d) remains open.
   ...
   **(d) Batch declaration `custom_json` fields — OPEN.** The batch declaration should include additional fields for robust batches. Candidate fields: `variant` (`"standard"` or `"robust"`), `locale` (wallet locale code), `endpoint_count` (number of embedded endpoints — not the endpoints themselves, which must not be published on-chain), `grace_window_expires` (ISO 8601 timestamp for when onboarding endpoints expire and the user should have transitioned to subscriber endpoints). The exact set of fields requires further design, particularly around what information is safe to publish on the public blockchain versus what must remain private.
