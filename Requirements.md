...
7. **Gift card generation pipeline for robust batches.** The existing `giftcard-generate.ts` script needs to support robust invite generation. Questions: (a) How are the **onboarding pool** proxy endpoint URLs provided as input — CLI flags, config file, or fetched from infrastructure? These must be drawn from the dedicated onboarding pool, not the subscriber pool. (b) Does the generation pipeline need access to the proxy infrastructure to validate endpoints are live before embedding them in cards? (c) How is the `locale` field determined — per-batch CLI flag matching the wallet locale codes? (d) Should the batch declaration `custom_json` include additional fields for robust batches (e.g. locale, endpoint count, grace window expiry)?
